...
class IntegerProgrammingWts:
    ...
    def TrainingForStratgy(self,startDate,EndDate):  
        ...
        problem = pl.LpProblem("Strategy Problem",pl.LpMinimize)
        
        df1_cum = np.exp(np.log1p(dfNew).cumsum())
        
        R_CumSum=[(df1_cum.iloc[len(dfNew)-1,i]-df1_cum.iloc[0,i]) for i in range(len(X.columns)+1)]
        X_cum=R_CumSum[:-1]
        Y_cum=R_CumSum[-1:]
    ...
    def IPWeightsClaculation(self):
        ...
        for i in range(len(self.df_wft)):
            if (self.DateTime_df[self.df_wft.iloc[i,0]:self.df_wft.iloc[i,1]]['SG Strategy'] > 0.01).any:
